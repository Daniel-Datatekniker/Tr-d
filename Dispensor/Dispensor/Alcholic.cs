...
namespace Dispensor
{
    class Alcholic : Consumer, IConsume
    {
        ...
        public void ConsumeDrink(object test)
        {
            while (true)
            {
                ...
                {
                    if (WareHouse.producedDrinks[i] != null && WareHouse.producedDrinks[i] is Beer)
                    {
                        WareHouse.beers[i] = null;
                        Console.WriteLine($"{Name} drank a beer");
                        Thread.Sleep(1000);
                    }
                }
                Monitor.Exit(_Lock); 
